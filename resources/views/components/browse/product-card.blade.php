@props(['image', 'title', 'desc', 'price', 'status', 'bank', 'likes', 'id' => 1])
...
<div class="bg-white rounded-md shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
    ...
    <div class="p-4">
        <a href="{{ route('product.detail', ['id' => $id]) }}" class="block">
            <h3 class="text-lg font-bold text-gray-800 mb-1 hover:text-green-600 transition-colors">{{ $title }}</h3>
        </a>
        ...
        <a href="{{ route('product.detail', ['id' => $id]) }}" class="block w-full text-center py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition-colors">Beli</a>
    </div>
</div> 